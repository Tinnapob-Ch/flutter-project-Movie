...
class Titanic extends StatefulWidget {
  ...
  const Titanic({Key? key, required this.callback}) : super(key: key);
...
class _TitanicState extends State<Titanic> {
  ...
  void initState() {
    super.initState();
    _controller = VideoPlayerController.asset('assets/Titanic.mp4');
...
void toggleAdd() {
  ...
  Future<void> getPreference() async {
    ...
    setState(() {
      isAdd = prefs.getBool('ADD_MOVIE11') ?? false;
      isFavorited = prefs.getBool('FAV_MOVIE11') ?? false;
  ...
  Future<void> saveValues(bool add, bool fav) async {
    final SharedPreferences prefs = await SharedPreferences.getInstance();
    await prefs.setBool('ADD_MOVIE11', add);
    await prefs.setBool('FAV_MOVIE11', fav);
    await prefs.setString('NAME_MOVIE11', 'Titanic');
